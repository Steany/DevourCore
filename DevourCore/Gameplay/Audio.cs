...
namespace DevourCore
{
	public class Audio
	{
		...
		internal static string MenuMusicSavedClipName = "";
		internal static bool MenuMusicRememberEnabled = false;
		private static readonly Dictionary<string, AudioClip> _clipCache = new Dictionary<string, AudioClip>(System.StringComparer.OrdinalIgnoreCase);
		...
		public void Initialize(MelonPreferences_Category prefs)
		{
			...
			MenuMusicSavedClipName = prefLastClipName.Value ?? "";
			MenuMusicRememberEnabled = prefRememberMusic.Value;
		...
		public void SetRememberMusic(bool enabled, MelonPreferences_Category prefs)
		{
			...
			MenuMusicRememberEnabled = enabled;
		...
		private IEnumerator InitMusicInScene()
		{
			...
			for (int i = 0; i < 180; i++)
			{
				...
				if (bgObj != null)
				{
					try { _bgMusicSource = bgObj.GetComponent<UnityEngine.AudioSource>(); } catch { _bgMusicSource = null; }
			...
			if (RememberMusic)
			{
				...
				{
					string current = null;
					try { current = _bgMusicSource.clip != null ? _bgMusicSource.clip.name : null; } catch { current = null; }

					if (string.IsNullOrEmpty(current) || !string.Equals(current, savedName, System.StringComparison.Ordinal))
					{
						try { _bgMusicSource.Stop(); } catch { }
			...
			while (_isTracking && _bgMusicSource != null)
			{
				if (RememberMusic)
				{
					...
					if (!string.IsNullOrEmpty(nameNow) && nameNow != _currentClipName)
					{
						...
						MenuMusicSavedClipName = _currentClipName;
		...
		private void TryApplySavedClip(string savedName)
		{
			...
			MenuMusicSavedClipName = savedName ?? "";
			...
			if (!_clipCache.TryGetValue(savedName, out target) || target == null)
			{
		...
		internal static bool TrySwapMenuMusicClip(AudioSource src)
		{
			if (src == null)
				return false;

			if (!MenuMusicRememberEnabled)
				return false;

			string saved = MenuMusicSavedClipName;
			if (string.IsNullOrEmpty(saved))
				return false;

			string objName = null;
			try { objName = src.gameObject != null ? src.gameObject.name : null; } catch { objName = null; }
			if (objName != "Background Music")
				return false;

			AudioClip currentClip = null;
			string currentName = null;
			try { currentClip = src.clip; currentName = currentClip != null ? currentClip.name : null; } catch { currentClip = null; currentName = null; }

			if (string.Equals(currentName, saved, System.StringComparison.Ordinal))
				return false;

			AudioClip target = null;
			if (!_clipCache.TryGetValue(saved, out target) || target == null)
			{
				var allClips = Resources.FindObjectsOfTypeAll<UnityEngine.AudioClip>();
				foreach (var clip in allClips)
				{
					if (clip != null && clip.name == saved)
					{
						target = clip;
						break;
					}
	...
	internal static class MenuMusicAudioPatches
	{
		public static void Play_Prefix(AudioSource __instance)
		{
			try
			{
				if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().name != "Menu")
					return;
				Audio.TrySwapMenuMusicClip(__instance);
			}
			catch { }
		}

		public static void PlayDelayed_Prefix(AudioSource __instance, float delay)
		{
			try
			{
				if (UnityEngine.SceneManagement.SceneManager.GetActiveScene().name != "Menu")
