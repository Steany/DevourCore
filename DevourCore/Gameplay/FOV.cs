...
namespace DevourCore
{
	public class FOV
	{
		...
		private float lastCustomFOV = 100f;
		private float originalGameFOV = 100f;
		private float originalMenuFOV = 45f;
		...
		public void Initialize(MelonPreferences_Category prefsCategory)
		{
			...
			prefLastFov = prefs.CreateEntry("LastFov", 100f);
			prefFovEnabled = prefs.CreateEntry("FovEnabled", false);
			prefFovToggleKey = prefs.CreateEntry("FovToggleKey", KeyCode.RightAlt);
			prefMenuFOV = prefs.CreateEntry("MenuFOV", 45f);
