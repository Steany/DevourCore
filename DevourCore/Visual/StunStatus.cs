...
namespace DevourCore
{
	public sealed class StunStatusIndicator
	{
		...
		private void DrawStatusBar(bool editing)
		{
			...
			{
				Vector2 tsize = statusStyle.CalcSize(new GUIContent(text));
				...
				float newW = tsize.x + horizPad;
				float newH = tsize.y + vertPad;

				
				if (posX < 0f)
					rect.x = (Screen.width - newW) * 0.5f;
		...
		private void HandleDrag(ref Rect rect)
		{
