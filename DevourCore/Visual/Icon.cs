...
namespace DevourCore
{
	public class Icon
	{
		// Ranks that CAN be tinted (70–666)
		...
		{
			prefHue = prefs.CreateEntry("Icon_Hue", 1f);
			...
			prefCurrentRankIndex = prefs.CreateEntry("Icon_CurrentRankIndex", RankCount - 1);
		...
		public void OnUpdate()
		{
			for (int i = rankImages.Count - 1; i >= 0; i--)
			{
				...
				// If it's not a known rank at all, drop it
				if (idx < 0 && !untinted)
				{
					rankImages.RemoveAt(i);
					continue;
		...
		// True for rank-1-9 Never Tint sprites
