...
namespace DevourCore
{
	public class Anticheat
	{
		...
		private float _alertPosX = 195f;
		private float _alertPosY = 0f;
		private bool _editPositionMode = false;
		private bool _isDragging = false;
		private Vector2 _dragOffset;
		private float _lastBoxWidth = 200f;
		private float _lastBoxHeight = 100f;
		...
		public Vector2 AlertAnchor => new Vector2(_alertPosX, _alertPosY);
		...
		public void Initialize(MelonPreferences_Category prefs)
		{
			...
			prefAlertPosX = prefs.CreateEntry("AnticheatAlertPosX", 195f);
			prefAlertPosY = prefs.CreateEntry("AnticheatAlertPosY", 0f);
		...
		public void ApplyFirstRunDefaults(MelonPreferences_Category prefsCategory)
		{
			...
			if (prefAlertPosX != null) prefAlertPosX.Value = 195f;
			if (prefAlertPosY != null) prefAlertPosY.Value = 0f;
			_alertPosX = 195f;
			_alertPosY = 0f;
			_players.Clear();
			_alerts.Clear();
		...
		public void OnGUI()
		{
			...
			ApplyHudScaleToStyles();
			EventType eventType = Event.current.type;
			if (eventType != EventType.Layout && eventType != EventType.Repaint)
			{
				if (_editPositionMode && (eventType == EventType.MouseDown || eventType == EventType.MouseDrag || eventType == EventType.MouseUp))
				{
					HandleDragEvents();
				}
				return;
			...
			if (!hasAlerts && !_editPositionMode)
				return;
			string titleText = Loc.GUI.SuspiciousPlayersTitle;
			...
			if (hasAlerts)
			{
				foreach (var alert in _alerts)
				{
					...
					lines.Add(Loc.GUI.BulletPrefix + alert.Text + timeStr);
				}
			}
			else if (_editPositionMode)
			{
				lines.Add(Loc.GUI.DragBoxHint);
			}
			if (lines.Count == 0)
				return;
			float s = HudManager.GlobalHudScale;
			float paddingH = 10f * s;
			float paddingV = 10f * s;
			float spacingTitleToText = 5f * s;
			...
			{
				Vector2 lineSize = alertTextStyle.CalcSize(new GUIContent(line));
				if (lineSize.x > maxTextWidth)
					maxTextWidth = lineSize.x;
			}
			float linesHeight = 0f;
			float contentWidth = maxTextWidth;
			for (int i = 0; i < lines.Count; i++)
			{
				linesHeight += alertTextStyle.CalcHeight(new GUIContent(lines[i]), contentWidth);
			}
			float boxWidth = maxTextWidth + paddingH * 2f;
			float boxHeight = paddingV + titleSize.y + spacingTitleToText + linesHeight + paddingV;
			_lastBoxWidth = boxWidth;
			...
			float contentX = paddingH;
			float contentW = boxWidth - paddingH * 2f;
			float y = paddingV;
			GUI.Label(new Rect(contentX, y, contentW, titleSize.y), titleText, alertTitleStyle);
			y += titleSize.y + spacingTitleToText;
			for (int i = 0; i < lines.Count; i++)
			{
				GUIContent c = new GUIContent(lines[i]);
				float lh = alertTextStyle.CalcHeight(c, contentW);
				GUI.Label(new Rect(contentX, y, contentW, lh), c.text, alertTextStyle);
				y += lh;
			}
			GUI.EndGroup();
		}
		...
		{
			if (!_editPositionMode) return;
			float boxWidth = _lastBoxWidth > 0f ? _lastBoxWidth : 200f * HudManager.GlobalHudScale;
			float boxHeight = _lastBoxHeight > 0f ? _lastBoxHeight : 100f * HudManager.GlobalHudScale;
			float maxX = Mathf.Max(0f, Screen.width - boxWidth);
			float maxY = Mathf.Max(0f, Screen.height - boxHeight);
			Rect areaRect = new Rect(_alertPosX, _alertPosY, boxWidth, boxHeight);
		...
		private void InitializeAlertStyles()
		{
			...
			alertTitleStyle.fontSize = Mathf.RoundToInt(14f * s);
			...
			alertTextStyle.fontSize = Mathf.RoundToInt(12f * s);
		...
		private void ApplyHudScaleToStyles()
		{
			float s = Mathf.Clamp(HudManager.GlobalHudScale, 1f, 1.4f);
			if (Mathf.Approximately(lastAppliedScale, s)) return;
			if (alertTitleStyle != null) alertTitleStyle.fontSize = Mathf.RoundToInt(14f * s);
			if (alertTextStyle != null) alertTextStyle.fontSize = Mathf.RoundToInt(12f * s);
			lastAppliedScale = s;
		}
