...
namespace DevourCore
{
    public class GUIManager
    {
        ...
        private void DrawMainWindow(int windowID, bool inMenu, bool inValidMap)
        {
            ...
            if (tabVisibility.ShowOptimize && DrawTabButton(Loc.GUI.Tab_Optimize, 0)) { _selectedTab = 0; resetConfirmPending = false; }
            if (tabVisibility.ShowHSV && DrawTabButton(Loc.GUI.Tab_HSV, 1)) { _selectedTab = 1; resetConfirmPending = false; }
            if (tabVisibility.ShowSpeedrun && DrawTabButton(Loc.GUI.Tab_Speedrun, 2)) { _selectedTab = 2; resetConfirmPending = false; }
            if (tabVisibility.ShowFOV && DrawTabButton(Loc.GUI.Tab_FOV, 3)) { _selectedTab = 3; resetConfirmPending = false; }
            if (tabVisibility.ShowAnticheat && DrawTabButton(Loc.GUI.Tab_Anticheat, 4)) { _selectedTab = 4; resetConfirmPending = false; }
            if (tabVisibility.ShowMenu && DrawTabButton(Loc.GUI.Tab_Menu, 5)) { _selectedTab = 5; resetConfirmPending = false; }
            if (DrawTabButton(Loc.GUI.Tab_Settings, 6)) { _selectedTab = 6; resetConfirmPending = false; }
        ...
        {
            DrawHeader(Loc.GUI.Header_Optimize);
            GUILayout.Label(Loc.GUI.Desc_Optimize, descriptionStyle);
            GUILayout.Space(10);
        ...
        private void DrawHSVTab()
        {
            ...
            GUILayout.Label(
                isEditingOutfit
                    ? Loc.GUI.Desc_OutfitColor
                    : Loc.GUI.Desc_IconColor,
                descriptionStyle);
            GUILayout.Space(10);
            ...
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            string swapText = isEditingOutfit ? Loc.GUI.Swap_ToOutfit : Loc.GUI.Swap_ToIcon;
        ...
        private void DrawSpeedrunTab(bool inMenu)
        {
            ...
            GUILayout.Label(Loc.GUI.Desc_Speedrun, descriptionStyle);
            GUILayout.Space(10);
            ...
            GUILayout.Label(Loc.GUI.Desc_AutoStart, descriptionStyle);
            GUILayout.Space(10);
        ...
        private void DrawFOVTab(bool inValidMap)
        {
            ...
            GUILayout.Label(Loc.GUI.Desc_FOV, descriptionStyle);
            GUILayout.Space(10);
        ...
        private void DrawAnticheatTab()
        {
            ...
            GUILayout.Label(Loc.GUI.Desc_Anticheat, descriptionStyle);
            GUILayout.Space(10);
            ...
            GUILayout.Label(string.Format(Loc.GUI.AnticheatStatusFormat, trackedCount, alertCount),
                descriptionStyle);
        }

        private void DrawMenuTab()
        {
            DrawHeader(Loc.GUI.Header_Menu);
            GUILayout.Label(Loc.GUI.Desc_Menu, descriptionStyle);
            GUILayout.Space(10);
            ...
            if (inLobby)
            {
                GUILayout.Label(Loc.GUI.InLobby_NoBgChange, descriptionStyle);
                GUILayout.Space(8);
            }
            ...
            if (menuTab.RememberMusic && !string.IsNullOrEmpty(menuTab.LastClipName))
                GUILayout.Label(string.Format(Loc.GUI.CurrentClipFormat, menuTab.LastClipName), descriptionStyle);
        }
        ...
        private float GetMapButtonWidth(int mapId)
        {
            switch (mapId)
            {
                case 5: return 65f;  // Inn
                case 1: return 70f;  // Town
                case 2: return 75f;  // Manor
                case 4: return 82f;  // Asylum
                case 7: return 90f;  // Carnival
                case 3: return 95f;  // Farmhouse
                case 6: return 115f; // Slaughterhouse
                default: return 88f;
            }
        }
        ...
        {
            DrawHeader(Loc.GUI.Header_Settings);
            GUILayout.Label(Loc.GUI.Desc_Settings, descriptionStyle);
            GUILayout.Space(15);
            ...
            GUILayout.Label(Loc.GUI.Desc_ThemeColor, descriptionStyle);
            GUILayout.Space(10);
            ...
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            string langLabel = Loc.Current == Language.EN ? "Language: English" : "语言: 中文";
            if (GUILayout.Button(GUIContent.none, buttonStyle, GUILayout.Width(200)))
            {
        ...
        private void InitializeStyles()
        {
            ...
            checkboxCheckedBg = MakeRoundedTextureWithBorder(64, 64, settingsTab.ThemeColorPrimary, settingsTab.ThemeColorDark, 3, 2);
