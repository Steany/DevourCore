...
namespace DevourCore
{
	public class GUIManager
	{
		...
		public void Update()
		{
			...
			{
				// Apply the "first run/off defaults" ONCE, then immediately stop enforcing them.
				// The old logic waited for the current values to *match* the defaults, which caused
				// user-changed settings (like Mute Weather Sounds) to be forced back off forever.
				ApplyInitialGuiDefaults();

				// Wait until the tabs exist at least once (so ApplyInitialGuiDefaults can actually run),
				// then mark as applied so user settings won't be overwritten on scene loads.
				if (optimizeTab != null || hsvTab != null || outfitTab != null)
				{
		...
		private void DrawGameplayTab(bool inMenu, bool inValidMap)
		{
			...
			GUILayout.Space(8);
			UnityEngine.GUI.enabled = inMenu;
			bool newMuteWeather = DrawCustomToggle(optimizeTab.MuteWeatherAudioSetting, Loc.GUI.Toggle_MuteWeather);
			if (newMuteWeather != optimizeTab.MuteWeatherAudioSetting && inMenu) optimizeTab.SetMuteWeatherAudio(newMuteWeather, prefs, inMenu, inValidMap);
			UnityEngine.GUI.enabled = true;
			GUILayout.Space(5);
		...
		private void DrawMenuTab()
		{
			...
			bool inMenuScene = menuTab.IsMenuSceneActive();
			bool canChangeMaps = menuTab.SceneEnabled && inMenuScene && !inLobby && !cooldown;
		...
		private bool DrawMapButton(string label, bool isSelected, float buttonWidth)
		{
			...
			if (Event.current.type == EventType.Repaint)
			{
				...
				if (!UnityEngine.GUI.enabled)
					UnityEngine.GUI.color = new Color(prev.r, prev.g, prev.b, prev.a * 0.55f);
